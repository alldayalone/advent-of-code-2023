...
main :: IO ()
main = do
  contents <- readFile "src/11_2/input.txt"
...
dist expanderX expanderY ((ch1, x1, y1), (ch2, x2, y2)) = distX + distY + expansionX + expansionY
  where 
    distX = abs $ x1 - x2
    distY = abs $ y1 - y2
    expansionX = (expansionMultiplier - 1) * sum (Vector.slice (min x1 x2) distX expanderX)
    expansionY = (expansionMultiplier - 1) * sum (Vector.slice (min y1 y2) distY expanderY)
