...
solve (Node ('.':s,   i , _       )) = solveMemoized (Node (s, i,      Nothing ))
solve (Node ('#':s,   _ , Just '.')) = 0
solve (Node ('#':s,   [], _       )) = 0
solve (Node ('#':s, 1:is, _       )) = solveMemoized (Node (s, is,     Just '.'))
solve (Node ('#':s, x:is, _       )) = solveMemoized (Node (s, x-1:is, Just '#')) 
solve (Node ('?':s,    i, Just '#')) = solveMemoized (Node ('#':s, i,  Nothing ))
solve (Node ('?':s,    i, Just '.')) = solveMemoized (Node ('.':s, i,  Nothing )) 
solve (Node ('?':s,    i, Nothing )) = solveMemoized (Node ('.':s, i,  Nothing )) + solveMemoized (Node ('#':s,i, Nothing))
